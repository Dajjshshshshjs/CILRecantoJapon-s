...
function completedLessons(userId) {
  return q.completedLessons.all(userId).map(row => row.lesson_id);
}
...
function publicUser(user) {
  return {
    ...
    total: user.total,
    errors: JSON.parse(user.errors_json || '[]'),
    streak: user.streak,
    lastDay: user.last_day,
    completedLessons: completedLessons(user.id)
  };
}
  ...
  try {
    ...
    if (
      ...
      if (
        ...
        return json(res, 400, {
          error: 'Selecione um semestre válido.'
        });
      }
      ...
      if (
        ...
        return json(res, 400, {
          error: 'Informe sua turma.'
        });
      }

      // FOTO OPCIONAL:
      // Se não enviar foto, o cadastro continua normalmente.
      // Se enviar, precisa ser JPG, PNG ou WEBP e ter até 2 MB.
      if (
        profilePhoto &&
        (
          !/^data:image\/(jpeg|png|webp);base64,/.test(profilePhoto) ||
          profilePhoto.length > 2_700_000
        )
      ) {
        return json(res, 400, {
          error: 'A foto deve estar em JPG, PNG ou WEBP e ter até 2 MB.'
        });
      }
    ...
    if (
      req.method === 'GET' &&
      url.pathname === '/api/curriculum'
    ) {
      const user = currentUser(req);

      if (!user) {
        return json(res, 401, {
          error: 'Faça login para acessar a trilha.'
        });
      }
    ...
    if (
      ...
      if (
        ...
        return json(res, 400, {
          error: 'A mensagem deve ter até 500 caracteres.'
        });
      }

      const result = q.addMessage.run(
        user.id,
        user.semester,
        user.classroom,
        content.trim()
    ...
    if (
      ...
      if (
        ...
        return json(res, 200, {
          students,
          books: q.listBooks.all(99),
          videos: q.listVideos.all(99),
          achievements:

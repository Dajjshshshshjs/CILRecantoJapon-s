...
const q = {
  ...
  updateProgress: db.prepare(
    'UPDATE users SET correct = ?, total = ?, errors_json = ?, streak = ?, last_day = ? WHERE id = ?'
  ),
  ...
  completedLessons: db.prepare(
    'SELECT lesson_id FROM lesson_progress WHERE user_id = ? ORDER BY completed_at'
  ),
  ...
  listBooks: db.prepare(
    'SELECT * FROM books WHERE semester <= ? ORDER BY semester, created_at DESC'
  ),

  listVideos: db.prepare(
    'SELECT * FROM videos WHERE semester <= ? ORDER BY semester, created_at DESC'
  ),

  listMessages: db.prepare(
    'SELECT messages.*, users.name FROM messages JOIN users ON users.id = messages.user_id WHERE messages.semester = ? AND messages.classroom = ? ORDER BY messages.id DESC LIMIT 80'
  ),

  listGrades: db.prepare(
    'SELECT * FROM grades WHERE user_id = ? ORDER BY created_at DESC'
  ),

  listUsers: db.prepare(
    'SELECT id, name, email, role, approval_status, semester, classroom, rejection_reason, correct, total, streak, created_at FROM users ORDER BY created_at'
  ),

  addBook: db.prepare(
    'INSERT INTO books (title, author, description, url, semester) VALUES (?, ?, ?, ?, ?)'
  ),

  addVideo: db.prepare(
    'INSERT INTO videos (title, url, description, semester) VALUES (?, ?, ?, ?)'
  ),

  addAchievement: db.prepare(
  ...
  addMessage: db.prepare(
    'INSERT INTO messages (user_id, semester, classroom, content) VALUES (?, ?, ?, ?)'
  ),

  setApproval: db.prepare(
    'UPDATE users SET approval_status = ?, rejection_reason = ? WHERE id = ?'
  ),

  setSemester: db.prepare(
    'UPDATE users SET semester = ? WHERE id = ?'
  ),

  addGrade: db.prepare(
    'INSERT INTO grades (user_id, subject, score, note) VALUES (?, ?, ?, ?)'
  ),
  ...
  addAudit: db.prepare(
    'INSERT INTO audit_events (actor_id, action, entity_type, entity_id, details_json) VALUES (?, ?, ?, ?, ?)'
  ),

  auditByUser: db.prepare(
    'SELECT * FROM audit_events WHERE actor_id = ? ORDER BY id DESC LIMIT 100'
  ),

  recentAudit: db.prepare(
    'SELECT audit_events.*, users.name AS actor_name FROM audit_events LEFT JOIN users ON users.id = audit_events.actor_id ORDER BY audit_events.id DESC LIMIT 150'
  )
};
...
function completedLessons(userId) {
  return q.completedLessons
    .all(userId)
    ...
    async (req, res) => {
      try {
        ...
        if (
          ...
          if (
            classroom.trim().length < 1 ||
            classroom.trim().length > 40
          ) {
            return json(res, 400, {
              error:
                'Informe sua turma.'
        ...
        if (
          ...
          return json(
            ...
            {
              'Set-Cookie':
                'nihongo_session=; HttpOnly; SameSite=Lax; Path=/; Max-Age=0'
            }
          );
        }
        ...
        if (
          ...
          return json(res, 200, {
            rules:
              ...
              'Respeite colegas, professores e os horários. Use o chat apenas para fins de estudo.'
          });
        }
        ...
        if (
          ...
          if (
            ...
            return json(res, 400, {
              error:
                'A mensagem deve ter até 500 caracteres.'
            });
          }
        ...
        if (
                      ...
                      signal:
                        s.total
                          ? `${Math.round(
                              ...
                              s.total *
                              100
                            )}% de acertos em ${s.total} respostas`
                          : 'Ainda sem atividade',
                      recommendation:
                        s.total &&
                        s.correct /
                          s.total <
                          0.6
                          ? 'Sugira revisão e prática fácil.'
          ...
          if (
            ...
            if (
              ...
              return json(
                ...
                {
                  error:
                    'Preencha título, autor e semestre.'
                }
              );
            }
          ...
          if (
            ...
            if (
              ...
              return json(
                ...
                {
                  error:
                    'Informe título, link de vídeo e semestre.'
                }
              );
            }
          ...
          if (
            ...
            if (
              ...
              return json(
                ...
                {
                  error:
                    'Informe o título da conquista.'
                }
              );
            }

            const result =
              q.addAchievement.run(
                title.trim(),
                description.trim(),
                icon.slice(
                  0,
          ...
          const studentMatch =
            url.pathname.match(
              /^\/api\/admin\/students\/(\d+)\/(approve|reject|semester|grade)$/
            );
            ...
            if (
              ...
              if (
                ...
                return json(
                  ...
                  {
                    error:
                      'A reprovação exige justificativa.'
                  }
                );
              }
            ...
            if (
              ...
              if (
                ...
                return json(
                  ...
                  {
                    error:
                      'Informe disciplina e nota de 0 a 10.'
                  }
                );
              }
        ...
        if (
          ...
          if (!user) {
            return json(
              ...
              {
                error:
                  'Faça login para concluir uma aula.'
              }
            );
          }
          ...
          if (
            ...
            return json(
              ...
              {
                error:
                  'Aula não encontrada na trilha.'
              }
            );
          }

          q.completeLesson.run(
            user.id,
            lessonId
          );
        ...
        if (
          ...
          if (!user) {
            return json(
              ...
              {
                error:
                  'Faça login para salvar o progresso.'
              }
            );
          }
